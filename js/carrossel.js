...
function rolarCarrossel(carrossel, direcao) {
  ...
  const limiteFinal = Math.max(0, carrossel.scrollWidth - carrossel.clientWidth);
...
export function configurarCarrosselMaisVendidos() {
  ...
  const permitePausaComMouse = window.matchMedia('(hover: hover) and (pointer: fine)').matches;
  ...
  function iniciarMovimentoAutomatico() {
    ...
    const naoPrecisaRolar = carrossel.scrollWidth <= carrossel.clientWidth + 1;

    if (prefereMovimentoReduzido || document.hidden || naoPrecisaRolar) {
      return;
    }
