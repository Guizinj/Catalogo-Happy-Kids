...
function criarCardProduto(produto, estaFavoritado) {
  ...
  const botaoFavorito = criarBotao({
    ...
    rotulo: (estaFavoritado ? 'Remover' : 'Adicionar') + ' ' + produto.nome + ' dos favoritos',
    acao: 'favoritar'
  });
  const iconeFavorito = criarIcone('favorite', ['favorite', estaFavoritado ? 'favoritado' : '']);
...
export function renderizarProdutos(
  ...
  produtos.forEach((produto) => {
    fragmento.appendChild(criarCardProduto(produto, codigosFavoritos.has(produto.codigo)));
  });
...
export function renderizarMaisVendidos(listaDeProdutos, listaFavoritos = []) {
  ...
  renderizarProdutos(produtos, false, listaFavoritos, 'carrossel-mais-vendidos');
...
export function renderizarListaFavoritos(favoritos = []) {
  ...
  lista.forEach((produto) => {
    ...
    const informacoes = criarElemento('div', {
      classes: ['info-favorito-mini']
    });
    informacoes.append(
      criarElemento('h4', { texto: produto.nome }),
      criarElemento('p', {
        texto: formatarMoeda(produto.preco * produto.quantidade)
      })
    );
...
export function favNavbar(quantidade) {
  ...
  botao.setAttribute(
    'aria-label',
    quantidade > 0 ? 'Abrir favoritos, ' + quantidade + ' itens' : 'Abrir favoritos'
  );
}
